...
PLOTSDIR = mkpath(joinpath(dirname(Base.active_project()), "docs", "plots-tvc"))
...
# DGP is a time-varying VAR(1) process
z0 = 0.1 * randn(rng, Float32, k)
A10 = Float32[0.7 0.3; 0.1 0.8]
A1t = zeros(Float32, k, k, T)
A1t[:, :, 1] = A10

z = zeros(Float32, k, T)
z[:, 1] = z0
e = 0.05 * randn(rng, Float32, k, T)
...
for t in 2:T 
    # f[:, t] = 0.5f[:, t-1] + ef[:, t]
    A1t[:,:,t] = A1t[:,:,t-1] + 0.0025*randn(rng, Float32, k, k)
    z[:, t] = γ + A1t[:,:,t] * z[:, t-1] + f[:, t] + e[:, t]
end
...
fig = Figure(size=(950, 650))
ax1 = Axis(fig[1, 1]; ylabel=L"\phi_{11t}")
l1 = lines!(ax1, A1t[1, 1, :])
fig
...
# Train the model!
tstate, lossfn = train!(tstate, (x, y), 200; λ=1)
